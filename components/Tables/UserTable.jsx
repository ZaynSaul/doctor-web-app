...
const UserTable = ({ userLists }) => {
  ...
  const deleteData = async () => {
    try {
      const res = await axios
        .delete("http://localhost:3000/api/users/" + uId.current)
        .then(() => {
      ...
      setUserLists(userData.filter((user) => user._id !== isDeleting));
    } catch (err) {
      console.log(err);
    }
    setConfirm(false);
  ...
  return (
    <div className="w-full bg-white overflow-x-auto relative shadow-sm border border-gray-100 sm:rounded-lg">
